...
function inplaytracker_deactivate()
{
	...
	if (class_exists('MybbStuff_MyAlerts_AlertTypeManager')) {
        ...
        $alertTypeManager->deleteByCode('inplaytracker_newthread_follow');
        $alertTypeManager->deleteByCode('inplaytracker_newreply');
        $alertTypeManager->deleteByCode('inplaytracker_newreply_follow');
	}
...
function inplaytracker_do_newreply() {
    ...
    $foruminfo['parentlist'] = ",".$forum['parentlist'].",";   
    $all_forums = $mybb->settings['inplaytracker_inplay'].",".$mybb->settings['inplaytracker_archive'];
    $selectedforums = explode(",", $all_forums);
...
function inplaytracker_alerts() {
	...
	class MybbStuff_MyAlerts_Formatter_InplaytrackerNewthreadFollowFormatter extends MybbStuff_MyAlerts_Formatter_AbstractFormatter
	{
	    ...
	    public function formatAlert(MybbStuff_MyAlerts_Entity_Alert $alert, array $outputAlert)
	    {
	        ...
	        return $this->lang->sprintf(
	            $this->lang->inplaytracker_newthread_follow,
                $outputAlert['from_user'],
	...
	if (class_exists('MybbStuff_MyAlerts_AlertFormatterManager')) {
		...
		$formatterManager->registerFormatter(
				new MybbStuff_MyAlerts_Formatter_InplaytrackerNewthreadFollowFormatter($mybb, $lang, 'inplaytracker_newthread_follow')
		);
    }
	...
	class MybbStuff_MyAlerts_Formatter_InplaytrackerNewReplyFollowFormatter extends MybbStuff_MyAlerts_Formatter_AbstractFormatter
	{
	    ...
	    public function formatAlert(MybbStuff_MyAlerts_Entity_Alert $alert, array $outputAlert)
	    {
            $alertContent = $alert->getExtraDetails();
	        return $this->lang->sprintf(
	            $this->lang->inplaytracker_newreply_follow,
                $outputAlert['from_user'],
                $alertContent['subject'],
	            $outputAlert['dateline']
	        );
	...
	if (class_exists('MybbStuff_MyAlerts_AlertFormatterManager')) {
		...
		$formatterManager->registerFormatter(
				new MybbStuff_MyAlerts_Formatter_InplaytrackerNewReplyFollowFormatter($mybb, $lang, 'inplaytracker_newreply_follow')
		);
	}
